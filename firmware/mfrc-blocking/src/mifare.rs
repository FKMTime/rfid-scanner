    ...
    S: embedded_hal::spi::SpiDevice,
{
    pub fn mifare_read(
        ...
        self.pcd_calc_crc(&tmp_buff, 2, &mut buff[2..])?;
    ...
    pub fn mifare_write(
        ...
        self.pcd_mifare_transceive(&cmd_buff, 2, false)?;
        self.pcd_mifare_transceive(buff, buff_size, false)?;
    ...
    pub fn mifare_ultralight_write(
        ...
        self.pcd_mifare_transceive(&mut cmd_buff, 6, false)?;
        Ok(())
    }

    pub fn mifare_transfer(&mut self, block_addr: u8) -> Result<(), PCDErrorCode> {
    ...
    pub fn mifare_decrement(&mut self, block_addr: u8, delta: u32) -> Result<(), PCDErrorCode> {
        self.mifare_two_step_helper(PICCCommand::PICC_CMD_MF_DECREMENT, block_addr, delta)
    }

    pub fn mifare_increment(&mut self, block_addr: u8, delta: u32) -> Result<(), PCDErrorCode> {
        self.mifare_two_step_helper(PICCCommand::PICC_CMD_MF_INCREMENT, block_addr, delta)
    }

    pub fn mifare_restore(&mut self, block_addr: u8) -> Result<(), PCDErrorCode> {
        self.mifare_two_step_helper(PICCCommand::PICC_CMD_MF_RESTORE, block_addr, 0)
    }

    pub fn mifare_get_value(&mut self, block_addr: u8) -> Result<u32, PCDErrorCode> {
        let mut buff = [0; 18];
        let mut size = 18;

        self.mifare_read(block_addr, &mut buff, &mut size)?;
        Ok(((buff[3] as u32) << 24)
            | ((buff[2] as u32) << 16)
            | ((buff[1] as u32) << 8)
            | (buff[0] as u32))
    ...
    pub fn mifare_set_value(&mut self, block_addr: u8, value: u32) -> Result<(), PCDErrorCode> {
        let mut buff = [0; 18];

        buff[0] = (value & 0xFF) as u8;
        buff[8] = (value & 0xFF) as u8;
        buff[1] = ((value & 0xFF00) as u16 >> 8) as u8;
        buff[9] = ((value & 0xFF00) as u16 >> 8) as u8;
        buff[2] = ((value & 0xFF0000) as u32 >> 16) as u8;
        buff[10] = ((value & 0xFF0000) as u32 >> 16) as u8;
        buff[3] = ((value & 0xFF000000) as u32 >> 24) as u8;
        buff[11] = ((value & 0xFF000000) as u32 >> 24) as u8;
        ...
        self.mifare_write(block_addr, &buff, 16)
    }
