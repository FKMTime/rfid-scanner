...
{
    pub fn picc_is_new_card_present(&mut self) -> Result<(), PCDErrorCode> {
    ...
    pub fn picc_halta(&mut self) -> Result<(), PCDErrorCode> {
        let mut buff = [0; 4];
        buff[0] = PICCCommand::PICC_CMD_HLTA;
        buff[1] = 0;

        self.pcd_calc_crc_single_buf(&mut buff, 2, 2)?;
        let res = self.pcd_transceive_data(&buff, 4, &mut [], &mut 0, &mut 0, 0, false);
    ...
    pub fn picc_select(&mut self, uid: &mut Uid, valid_bits: u8) -> Result<(), PCDErrorCode> {
        ...
        while !uid_complete {
            ...
            while !select_done {
                if current_level_known_bits >= 32 {
                    ...
                    self.pcd_calc_crc_single_buf(&mut buff, 7, 7)?;
                ...
                match res {
                    ...
                    Err(PCDErrorCode::Collision) => {
                        let value_of_coll_reg = self.read_reg(PCDRegister::CollReg)?;
